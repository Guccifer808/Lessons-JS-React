window.addEventListener("DOMContentLoaded", () => {
  ...
  // Lesson 62 - Slider another method
  let slidesWrapper = document.querySelector(".offer__slider-wrapper"),
    slidesField = document.querySelector(".offer__slider-inner"),
    //gettng width of slides window
    width = window.getComputedStyle(slidesWrapper).width;

  //setting all slides width
  ...
  next.addEventListener("click", () => {
    ...
    if (offset == +width.slice(0, width.length - 2) * (slides.length - 1)) {
      ...
      offset += +width.slice(0, width.length - 2); // next button to add next slide's width
    }
    slidesField.style.transform = `translateX(-${offset}px)`;

    // end-start of slides logic
  ...
  // Previous method SLider

  // //initializing slide
  // showSlides(slideIndex);

  // // TOTAL COUNTER: adding 0 is < 10 to slide total counter, else
  // if (slides.length < 10) {
  //   total.textContent = `0${slides.length}`;
  // } else {
  //   total.textContent = slides.length;
  ...
  // // if last slide is selected and clicked next = set to 1st slide
  ...
  //   // CURRENT COUNTER: adding 0 is < 10 to slide current counter, else
  //   if (slides.length < 10) {
  //     current.textContent = `0${slideIndex}`;
  //   } else {
  //     current.textContent = slideIndex;
